...
if __name__ == '__main__':
    ...
    '''
        ...
        Distance to line in road: ~40.2336
    '''
    ROAD_DIST = 0.025

    '''
        Speed limit of urban freeways in India (50-65 KMH)
    '''
    HIGHWAY_SPEED_LIMIT = 25
    ...
    while True:
        ...
        cv2.line(frame,(0, Y_THRESH),(2000, Y_THRESH),(255,0,0),2)
                ...
                if len(vehicle.trace) > 1:
                    ...
                    try:
                        ...
                        if trace_y <= Y_THRESH + 5 and trace_y >= Y_THRESH - 5 and not vehicle.passed:
                            ...
                            vehicle.mph = (ROAD_DIST / time_dur)
                            ...
                            if vehicle.mph > HIGHWAY_SPEED_LIMIT:
                                ...
                                cv2.circle(orig_frame, (int(trace_x), int(trace_y)+1), 20, (0, 0, 255), 2)
                                cv2.putText(orig_frame, 'MPH: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)), font, 3, (0, 0, 255), 1, cv2.LINE_AA)
                        ...
                        if vehicle.passed:
                            ...
                            cv2.putText(frame, 'MPH: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)+1), font, 2, (0, 255, 255), 1, cv2.LINE_AA)
                        else:
    ...
    #for file in glob.glob('images/speeding_*.png'):
        #os.remove(file)